...
export default function AdminLayout({ children, active }: { children: React.ReactNode; active?: string }) {
 ...
 <Link href={role === 'brand' ? '/admin/products' : '/admin/dashboard'} className="font-bold text-amber-400 text-base">VAPOR-X</Link>
 <button 
 ...
 {/* 移动端下拉导航 */}
 {mobileMenuOpen && (
   <div className="bg-gray-900 border-t border-gray-800 pb-4 max-h-[80vh] overflow-y-auto">
     ...
     <div className="border-t border-gray-800 mt-2 pt-2 px-4 space-y-1">
       <Link href="/admin/change-password" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-gray-500 rounded-lg">
         <span>🔑</span>
         <span>修改密码</span>
       </Link>
       <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-gray-500 rounded-lg">
         <span>←</span>
         <span>返回前台</span>
       </Link>
       <button onClick={() => { document.cookie = 'admin_token=;path=/;max-age=0'; document.cookie = 'admin_role=;path=/;max-age=0'; window.location.href = '/admin/login'; }} 
         className="flex items-center gap-2 px-3 py-2 text-sm text-red-400 rounded-lg w-full text-left">
         <span>🚪</span>
         <span>退出登录</span>
       </button>
     </div>
