...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        showMsg('✅ 产品已更新成功！', 'success')
        setTimeout(() => router.push('/admin/products'), 1000)
      } else {
        const err = await res.json()
        showMsg(`❌ 保存失败: ${err.error}`, 'error')
      }
    } catch (err: any) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 mt-1">编辑产品</h1>
          </div>
          <button onClick={handleDelete} className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm font-medium">
            🗑️ 删除
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
          ...
          <div className="border-t pt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">🔬 产品参数（电子烟专用）</h3>
          ...
          <div className="border-t pt-6 flex gap-3">
            <button type="submit" disabled={saving || uploading}
              className="px-8 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:bg-gray-300 text-black font-semibold rounded-lg transition flex items-center gap-2">
              {saving ? (
                <>
                  <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
