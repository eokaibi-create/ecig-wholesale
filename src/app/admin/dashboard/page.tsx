      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              return (
                <div key={cat.id} className="mb-3">
                  <div className="flex justify-between text-sm mb-1">
                    <Link href={`/admin/products?category=${cat.slug}`} className="text-gray-700 hover:text-amber-600">{cat.name}</Link>
            ...
            <Link href="/admin/products" className="mt-4 inline-block text-sm text-amber-600 hover:text-amber-700 font-medium">管理产品 →</Link>
          </div>
                ...
                {recentInquiries.map((inq) => (
                  <div key={inq.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-gray-900 truncate">{inq.customer.name}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {[
              ...
              <Link key={item.href} href={item.href}
                className="flex flex-col items-center gap-1.5 p-4 rounded-xl border border-gray-100 hover:border-amber-300 hover:bg-amber-50/50 transition text-center">
