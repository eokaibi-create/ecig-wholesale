      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {stats.lowStockCount > 0 && (
            <span className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-700 text-sm rounded-full border border-red-200">
              <span>⚠️</span> {stats.lowStockCount} 个产品库存不足
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: '品牌', value: stats.brandCount, color: 'from-violet-500 to-violet-600', icon: '🏷️', href: '/admin/brands' },
...
function FullDashboard() {
  ...
  useEffect(() => {
    fetch('/api/admin/me')
      ...
      .then(me => {
        fetch('/api/admin/dashboard-stats')
          .then(res => res.json())
          .then(d => { setData({ ...d, role: me.role }); setLoading(false) })
          .catch(() => setLoading(false))
      })
      .catch(() => setLoading(false))
  }, [])
  ...
  const statCards = [
    ...
    { label: '品牌', value: s.brandCount, color: 'from-violet-500 to-violet-600', icon: '🏷️', href: '/admin/brands' },
    { label: '客户数', value: s.customerCount, color: 'from-purple-500 to-purple-600', icon: '👥', href: '/admin/customers' },
    { label: '询价单', value: s.inquiryCount, sub: `新询价 ${s.newInquiryCount || 0}`, color: 'from-amber-500 to-orange-600', icon: '📋', href: '/admin/inquiries' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">VAPOR-X 批发管理平台数据概览</p>
          </div>
        </div>
        ...
        {s.categoryStats && s.categoryStats.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">分类产品统计</h2>
