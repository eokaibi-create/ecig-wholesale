...
export type TokenPayload = AdminTokenPayload | UserTokenPayload | CustomerTokenPayload
...
export function getTokenFromCookie(request: NextRequest): string | null {
  return request.cookies.get('admin_token')?.value || request.cookies.get('customer_token')?.value || null
}
...
/**
 * 解析客户 token（仅限 customer 类型）
 */
export async function parseCustomerToken(request: NextRequest): Promise<CustomerTokenPayload | null> {
